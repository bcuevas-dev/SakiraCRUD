$(document).ready(function () {
    const tabla = $('#tablaStaff').DataTable({
        processing: true,
        serverSide: false,
        ajax: {
            url: '/Staff/GetAll',
            type: 'GET',
            dataSrc: 'data'
        },
        columns: [
            { data: 'nombreCompleto' },
            { data: 'email' },
            { data: 'username' },
            { data: 'direccion' },
            { data: 'tienda' },
            {
                data: 'activo',
                render: function (data) {
                    return data ? '<span class="badge bg-success">Sí</span>' : '<span class="badge bg-secondary">No</span>';
                }
            },
            { data: 'lastUpdate' },
            {
                data: 'staffId',
                render: function (data) {
                    return `
                        <button class="btn btn-warning btn-sm edit-btn" data-id="${data}">Editar</button>
                        <button class="btn btn-danger btn-sm delete-btn" data-id="${data}">Eliminar</button>
                    `;
                },
                orderable: false
            }
        ],
        language: {
            url: "//cdn.datatables.net/plug-ins/1.13.6/i18n/es-ES.json"
        }
    });

    // Agregar nuevo
    $('#btnAgregar').click(function () {
        $.get('/Staff/Create', function (html) {
            $('#modalContent').html(html);
            $('#modalStaff').modal('show');
            bindForm();
        });
    });

    // Editar
    $('#tablaStaff').on('click', '.edit-btn', function () {
        const id = $(this).data('id');
        $.get('/Staff/Edit/' + id, function (html) {
            $('#modalContent').html(html);
            $('#modalStaff').modal('show');
            bindForm();
        });
    });

    // Eliminar
    $('#tablaStaff').on('click', '.delete-btn', function () {
        const id = $(this).data('id');
        Swal.fire({
            title: '¿Está seguro?',
            text: "Esta acción eliminará el empleado.",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Sí, eliminar',
            cancelButtonText: 'Cancelar'
        }).then((result) => {
            if (result.isConfirmed) {
                $.post('/Staff/Delete/' + id, function (res) {
                    if (res.success) {
                        toastr.success(res.message);
                        tabla.ajax.reload();
                    } else {
                        toastr.error(res.message);
                    }
                });
            }
        });
    });

    // Bind del formulario
    function bindForm() {
        $('#formStaff').submit(function (e) {
            e.preventDefault();
            const form = $(this);
            $.ajax({
                url: form.attr('action'),
                method: form.attr('method'),
                data: form.serialize(),
                success: function (res) {
                    if (res.success) {
                        $('#modalStaff').modal('hide');
                        toastr.success(res.message);
                        tabla.ajax.reload();
                    } else {
                        if (typeof res === 'string') {
                            $('#modalContent').html(res);
                            bindForm();
                        } else {
                            toastr.error(res.message);
                        }
                    }
                },
                error: function () {
                    toastr.error("Error inesperado al procesar.");
                }
            });
        });
    }
});
